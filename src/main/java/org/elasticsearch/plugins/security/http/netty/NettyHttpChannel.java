...
public class NettyHttpChannel implements HttpChannel {
	...
	public NettyHttpChannel(
			final NettyHttpServerTransport transport,
	...
	public void sendResponse(final RestResponse response) {
		...
		final boolean http10 = this.request.getProtocolVersion().equals(
				HttpVersion.HTTP_1_0);
		final boolean close = HttpHeaders.Values.CLOSE
				.equalsIgnoreCase(this.request
						.getHeader(HttpHeaders.Names.CONNECTION))
				|| http10
				&& !HttpHeaders.Values.KEEP_ALIVE.equalsIgnoreCase(this.request
						.getHeader(HttpHeaders.Names.CONNECTION));

		// Build the response object.
		final HttpResponseStatus status = this.getStatus(response.status());
		...
		if (RestUtils.isBrowser(this.request
				.getHeader(HttpHeaders.Names.USER_AGENT))) {
			if (this.transport.settings().getAsBoolean("http.cors.enabled",
					true)) {
				// Add support for cross-origin Ajax requests (CORS)
				resp.addHeader("Access-Control-Allow-Origin", this.transport
						.settings().get("http.cors.allow-origin", "*"));
				if (this.request.getMethod() == HttpMethod.OPTIONS) {
					// Allow Ajax requests based on the CORS "preflight" request
					resp.addHeader("Access-Control-Max-Age", this.transport
							.settings().getAsInt("http.cors.max-age", 1728000));
					resp.addHeader(
							"Access-Control-Allow-Methods",
							this.transport.settings().get(
					...
					resp.addHeader(
							...
							this.transport
									.settings()
									.get("http.cors.allow-headers",
											"X-Requested-With, Content-Type, Content-Length"));
				}
			}
		}

		final String opaque = this.request.getHeader("X-Opaque-Id");
		if (opaque != null) {
			resp.addHeader("X-Opaque-Id", opaque);
		}

		// Add all custom headers
		final Map<String, List<String>> customHeaders = response.getHeaders();
		if (customHeaders != null) {
			for (final Map.Entry<String, List<String>> headerEntry : customHeaders
					.entrySet()) {
		...
		try {
			if (response instanceof XContentRestResponse) {
				// if its a builder based response, and it was created with a
				// CachedStreamOutput, we can release it
				// after we write the response, and no need to do an extra copy
				// because its not thread safe
				final XContentBuilder builder = ((XContentRestResponse) response)
						.builder();
				...
				if (response.contentThreadSafe()) {
					buf = ChannelBuffers.wrappedBuffer(response.content(),
							response.contentOffset(), response.contentLength());
				} else {
					buf = ChannelBuffers.copiedBuffer(response.content(),
							response.contentOffset(), response.contentLength());
				}
			}
		...
		if (response.prefixContent() != null
				|| response.suffixContent() != null) {
			ChannelBuffer prefixBuf = ChannelBuffers.EMPTY_BUFFER;
			if (response.prefixContent() != null) {
				prefixBuf = ChannelBuffers.copiedBuffer(
						response.prefixContent(),
						response.prefixContentOffset(),
						response.prefixContentLength());
			}
			ChannelBuffer suffixBuf = ChannelBuffers.EMPTY_BUFFER;
			if (response.suffixContent() != null) {
				suffixBuf = ChannelBuffers.copiedBuffer(
						response.suffixContent(),
						response.suffixContentOffset(),
		...
		if (this.transport.resetCookies) {
			final String cookieString = this.request
					.getHeader(HttpHeaders.Names.COOKIE);
			if (cookieString != null) {
				...
				if (!cookies.isEmpty()) {
					...
					resp.addHeader(HttpHeaders.Names.SET_COOKIE,
		...
		// Write the response.
		final ChannelFuture future = this.channel.write(resp);
	...
	private HttpResponseStatus getStatus(final RestStatus status) {
		switch (status) {
		case CONTINUE:
