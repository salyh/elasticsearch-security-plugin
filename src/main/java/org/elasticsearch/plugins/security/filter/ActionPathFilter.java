...
public class ActionPathFilter extends SecureRestFilter {
	...
	public void processSecure(final TomcatHttpServerRestRequest request,
		...
		if (SecurityUtil
				.stringContainsItemFromListAsTypeOrIndex(
						request.path(),
						securityService.isStrictModeEnabled() ? SecurityUtil.BUILT_IN_READ_COMMANDS_STRICT
								: SecurityUtil.BUILT_IN_READ_COMMANDS_LAX)) {
			log.warn("Index- or Typename should not contains search commands like "
					+ Arrays.toString(securityService.isStrictModeEnabled() ? SecurityUtil.BUILT_IN_READ_COMMANDS_STRICT
							: SecurityUtil.BUILT_IN_READ_COMMANDS_LAX));
		}
		...
		if (SecurityUtil
				.stringContainsItemFromListAsTypeOrIndex(
						request.path(),
						securityService.isStrictModeEnabled() ? SecurityUtil.BUILT_IN_WRITE_COMMANDS_STRICT
								: SecurityUtil.BUILT_IN_WRITE_COMMANDS_LAX)) {
			log.warn("Index- or Typename should not contains write commands like "
					+ Arrays.toString(securityService.isStrictModeEnabled() ? SecurityUtil.BUILT_IN_WRITE_COMMANDS_STRICT
							: SecurityUtil.BUILT_IN_WRITE_COMMANDS_LAX));
		}
		...
		try {
			...
			final PermLevel permLevel = new PermLevelEvaluator(
					securityService.getXContentSecurityConfiguration(getType(),
							getId())).evaluatePerm(
					SecurityUtil.getIndices(request),
					SecurityUtil.getTypes(request),
					getClientHostAddress(request),
					new TomcatUserRoleCallback(request.getHttpServletRequest(),
							securityService.getSettings().get(
									"security.ssl.userattribute")));
			...
			if (permLevel.ordinal() < PermLevel.READWRITE.ordinal()
					&& SecurityUtil.isWriteRequest(request,
							securityService.isStrictModeEnabled())) {
				SecurityUtil.send(request, channel, RestStatus.FORBIDDEN,
			...
			massageKibanaRequest(request, channel);
			// Ram Kotamaraja - END
	...
	private void massageKibanaRequest(
		...
		try {
			...
			String reqContent = new String(request.getContent().toBytes());
